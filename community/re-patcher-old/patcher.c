...
// void gen_correct(){
//     const uint8_t *flag = "CTFlearn{p4tching_4nd_p4tching}";
...
bool check_flag(char *flag, size_t flag_len){
    ...
    for(int i = 0; i < flag_len; i++){
        // printf("%c", fake_flag[i] ^ xor_key[i]);
...
void preinit(int argc, char **argv, char **envp) {
    // printf("%s\n", __FUNCTION__);
    char *main_addr = (char *)(main + 0x1b); // after calling atexit
...
// replace call to check func in destructor with 
void init(int argc, char **argv, char **envp) {
    // printf("%s\n", __FUNCTION__);
    uint32_t *destructor_addr = (uint32_t *)((uint64_t)destructor + 0x28 + 1); // call <check_flag>
